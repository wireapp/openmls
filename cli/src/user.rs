...
impl User {
    ...
    pub async fn new(username: String) -> Self {
        ...
        let out = Self {
            ...
            identity: RwLock::new(Identity::new(CIPHERSUITE, &crypto, username.as_bytes()).await),
    ...
    pub async fn key_packages(&self) -> Vec<(Vec<u8>, KeyPackage)> {
        vec![(
            self.identity
                ...
                .as_slice()
                .to_vec(),
            self.identity.read().await.kp.clone(),
        )]
    }
    ...
    pub async fn read_msgs(&self, group_name: String) -> Result<Option<Vec<String>>, String> {
    ...
    async fn process_protocol_message(
        ...
        let processed_message = match mls_group.process_message(&self.crypto, message).await {
            Ok(msg) => msg,
            Err(e) => {
                log::error!(
    ...
    pub async fn update(&mut self, group_name: Option<String>) -> Result<Vec<String>, String> {
            ...
            match message.extract() {
                ...
                MlsMessageInBody::PrivateMessage(message) => {
                    if let Ok(Some(message)) = self
                        .process_protocol_message(group_name.as_deref(), message.into())
                        .await
                    {
        ...
        for c in self.backend.list_clients().await?.drain(..) {
            if c.id != self.identity.read().await.identity()
                && self
                    .contacts
                    .insert(
                        c.id.clone(),
    ...
    async fn join_group(&self, welcome: Welcome) -> Result<(), String> {
        ...
        match self.groups.write().await.insert(group_id, group) {
