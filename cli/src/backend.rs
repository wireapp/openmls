...
impl Backend {
    ...
    pub async fn get_client(&self, client_id: &[u8]) -> Result<ClientKeyPackages, String> {
        let mut url = self.ds_url.clone();
    ...
    pub async fn send_welcome(&self, welcome_msg: &Welcome) -> Result<(), String> {
        let mut url = self.ds_url.clone();
    ...
    pub async fn recv_msgs(&self, user: &User) -> Result<Vec<Message>, String> {
        let mut url = self.ds_url.clone();
